...
def get_Lesson_Tags(module_slug):
    found_lesson = Lesson.objects.filter(slug=module_slug)

    if found_lesson:
        return found_lesson.tags.all()
    else:
        return None
...
def get_Section_Tags(section_slug):
    ...
    found_section = Section.objects.filter(slug=section_slug)
